...
The process that takes place from the moment Splunk begins parsing logs until they are written to an index as individual events—where each event represents something that happend at a specific point in time—is known as **index-time processing**.

Beyond indexing, Splunk also requires instructions on how to structure and normalize logs for analysis. This process, known as **search-time processing**, occurs from the moment a search is initiated until it successfully completes.
...
Additionally, it is essential to consider the **content** in the logs to get a sense of they fall into different categories, and your which is to 

**categorize logs appropriately**. Even within the same vendor, application, and module combination, multiple sources (e.g., two log files) may exist with the same log format but contain different content, meaning they should be categorized separately. Proper categorization ensures that events can be easily filtered in Splunk, allowing users to efficiently find the desired subset of data.
